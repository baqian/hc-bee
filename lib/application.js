...
class Application extends BaseApp {
  ...
  init(frameworkConfig, frameworkOptions, appConfig, envConfig) {
    ...
    this.server.on('error', (err) => {
      log.error('app listen failed', err.code, err.message);
    });
    ...
    // 附加responseWraper 到 res.wrapper;
    this.use(function(req, res, next){
      res.wrapper = responseWraper(req, res, next);
      next();
    });
    /**
     * 挂载配置的middleware
  ...
  run(cb, config) {
    ...
    /**
     * @param {String|Number|Object|Error} err
     */
    this.express.use(function (err, req, res, next) {   // eslint-disable-line
      ...
      if (err instanceof Error || typeof err === 'object') {  // Error|{code, message}
        ...
        message = err.message;
        httpCode = err.status || err.statusCode || utils.map[String(code).toUpperCase()] || 500;
        ...
        code = 'ERROR';
        message = err;
        httpCode = utils.map[String(message).toUpperCase()] || 500;
      }
  ...
  mountMiddleWare() {
    ...
    middlewareList.forEach(function (mid) {
      ...
      mds.forEach((middleware) => {
         ...
         */
        if (middleware.constructor.name !== 'GeneratorFunction') {
          let customCfg = middlewareConfig[id] ? middlewareConfig[id].config : {};
        ...
        try {
          ...
          self.exit('mount middleware exception, middleware:' + id + ' ' + e.message);
        }
      });
      self.plugins.mountedMiddleware.push(id);
